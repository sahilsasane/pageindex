...
def extract_toc_content(content, model=None):
    ...
    response, finish_reason = _result
    ...
    if not isinstance(_result2, tuple):
        raise ValueError(
            f"Unexpected response from ChatGPT_API_with_finish_reason: {_result2}"
        )
    new_response, finish_reason = _result2
    response = (response or "") + (new_response or "")
    if_complete = check_if_toc_transformation_is_complete(content, response, model)
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        if not isinstance(_result3, tuple):
            raise ValueError(
                f"Unexpected response from ChatGPT_API_with_finish_reason: {_result3}"
            )
        new_response, finish_reason = _result3
        response = (response or "") + (new_response or "")
        if_complete = check_if_toc_transformation_is_complete(content, response, model)
...
def toc_transformer(toc_content, model=None):
    ...
    last_complete, finish_reason = _toc_result
...
def generate_toc_continue(toc_content, part, model: str | None = "gpt-4o-2024-11-20"):
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    assert isinstance(toc_with_page_number, list), (
        "generate_toc_init must return a list"
    )
...
def check_toc(page_list, opt=None):
    assert opt is not None, "opt config is required"
...
def single_toc_item_index_fixer(
    section_title, content, model: str | None = "gpt-4o-2024-11-20"
):
    tob_extractor_prompt = """
